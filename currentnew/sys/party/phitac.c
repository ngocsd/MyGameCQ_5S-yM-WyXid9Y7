	...
	else
	{
		...
		else
		{
			if (mixTime[TIME_HOUR] == 1 || mixTime[TIME_HOUR] == 3 || mixTime[TIME_HOUR] == 5 || mixTime[TIME_HOUR] == 7 || (mixTime[TIME_HOUR] == 9 && mixTime[TIME_MIN] > 30) || (mixTime[TIME_HOUR] == 11 && mixTime[TIME_MIN] > 30) //Nếu giờ hiện tại là: 1 , 3, 5, 7h thì Kết thúc hàm -> ko có Phi Tặc
				|| (mixTime[TIME_HOUR] == 13 && mixTime[TIME_MIN] > 30) || (mixTime[TIME_HOUR] == 15 && mixTime[TIME_MIN] > 30) || (mixTime[TIME_HOUR] == 17 && mixTime[TIME_MIN] > 30)													// Nếu ko có dòng > 30 thì 9h59 vẫn sẽ có phỉ còn nếu có thì khi bật GSE lên 9h30 sẽ ko có phỉ
				|| (mixTime[TIME_HOUR] == 19 && mixTime[TIME_MIN] > 30) || (mixTime[TIME_HOUR] == 21 && mixTime[TIME_MIN] > 30) || (mixTime[TIME_HOUR] == 23 && mixTime[TIME_MIN] > 30))
...
void win_bonus_callout(object me, object who)
{
	...
	for (i = 0; i < size; i++)
		if (team[i])
		{
			...
			"sys/sys/test_db"->add_yuanbao(team[i], level * 5 / size);
			write_user(team[i], sprintf(ECHO "Bạn tiêu diệt Phi Tặc cấp %d và nhận được %d Ngân Bảo !", level, level * 5 / size));
