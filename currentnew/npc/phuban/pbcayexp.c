...
void do_look(object who)
{
	...
	result += sprintf(ESC "Phải! Tiến vào rèn luyện (Tiêu hao 1M ngân lượng)\ntalk %x# welcome.1\n", getoid(me));
...
void do_welcome2(object me, string arg)
{
	...
	case 1:
		...
		{
			send_user(who, "%c%c%w%s", ':', 3, get_char_picid(), sprintf(" %s :\n Số ngân lượng của bạn không đủ 100.000 !" ESC "Rời khỏi", get_name()));
			return;
		}
		if (who->get_level() < 20)
		{
			send_user(who, "%c%c%w%s", ':', 3, get_char_picid(), sprintf(" %s :\n Người chơi trên 20 cấp mới được vào !" ESC "Rời khỏi", get_name()));
			return;
		}
		if (who->get_save_2("timepbexp") == iGio && !is_god(who))
		{
